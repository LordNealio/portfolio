...
export const backgroundQuestions: BackgroundQuestion[] = [
  ...
  // ── Multigenerational Black American lineage (the historic pre-1965 community) ──
  // Slavery is NOT an eligibility requirement; it appears only as one item among many
  // in the family-experiences checklist. These items let analysis group participants
  // by lineage cohort (see docs/STUDY_PHASE2_PLAN.md) using memory that typically
  // reaches parents, grandparents, and sharecroppers more readily than slavery.
  { id: "bg_community_identify", q: "Does your family identify with a multigenerational Black American community established in the United States?", type: "single", options: ["Yes", "No", "Unsure"].map((l) => ({ id: l, label: l })).concat([PNA]) },
  { id: "bg_grandparents_us_born", q: "How many of your grandparents were born in the United States?", type: "single", options: ["0", "1", "2", "3", "4", "Unsure"].map((l) => ({ id: l, label: l })).concat([PNA]) },
  { id: "bg_grandparents_pre1965", q: "How many of your grandparents came from families already established in the United States before 1965?", type: "single", options: ["0", "1", "2", "3", "4", "Unsure"].map((l) => ({ id: l, label: l })).concat([PNA]) },
  { id: "bg_established_pre1965", q: "To the best of your knowledge, was your family established in the United States before 1965?", type: "single", options: ["Yes", "No", "Unsure"].map((l) => ({ id: l, label: l })).concat([PNA]) },
  { id: "bg_family_experiences", q: "Which experiences are part of your known family history? (select all that apply)", type: "multi", options: ["Sharecropping", "Segregation or Jim Crow", "The Great Migration", "Civil-rights activism", "Historically Black churches", "Historically Black neighborhoods or towns", "Military service", "Agricultural labor", "Industrial labor", "Free Black community before the Civil War", "Enslavement", "Indigenous American ancestry or community connection", "Immigration after 1965", "Other", "Unsure"].map((l) => ({ id: l, label: l })).concat([PNA]) },
  { id: "bg_earliest_known", q: "What is the earliest generation or period you can account for in your family's history in the Americas?", type: "single", options: ["My parents' generation", "My grandparents' generation", "My great-grandparents' generation", "The Jim Crow era (roughly 1877–1965)", "The 1800s / Reconstruction or earlier", "The era of slavery", "Unsure"].map((l) => ({ id: l, label: l })).concat([PNA]) },
  { id: "bg_history_certainty", q: "How certain are you about your family's history in the United States?", type: "single", options: ["Very certain", "Somewhat certain", "Somewhat uncertain", "Very uncertain"].map((l) => ({ id: l, label: l })).concat([PNA]) },
  { id: "bg_self_terms", q: "What words do you personally use to describe your historical or cultural community? (optional)", type: "text" },
